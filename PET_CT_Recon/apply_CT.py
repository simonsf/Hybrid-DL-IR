...
from PET_CT_Recon.impl.pyimpl import ReconCT


def load_model(model_dir, gpu_id):
    ...
    param_file = os.path.join(model_dir, 'param_CT.pt')
    ...
    net_module = load_net_module("CT_recon_unet")
    net = net_module.ReconCTUNet(5, 1, n_RRG=2)
...
def inference(input_path, model_folder, output_folder, gpu_id=0):
    ...
    model = load_model(model_dir, gpu_id)
    ...
    try:
        prediction_array = ReconCT(image_array, model)
...
def do_inference():
    ...
    parser.add_argument('-i', '--input', type=str, default="v1_data/CT/1/low_dose.nii.gz", help="input folder/file for intensity images")
    parser.add_argument('-m', '--model', type=str, default="", help="model root folder")
    parser.add_argument('-o', '--output', type=str, default="v1_data/CT", help="output folder")
